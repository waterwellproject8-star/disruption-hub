...
// ── ROUTINE ACTIONS — one tap, instant log ────────────────────────────────────
const ROUTINE_ACTIONS = [
  { id:'arrived_collection', label:'At Collection', icon:'📍', color:'#3b82f6', status:'at_collection', toast:'Arrived at collection logged' },
  { id:'loading_complete',   label:'Loaded',        icon:'✅', color:'#00e5b0', status:'loaded',         toast:'Load secured — logged' },
  { id:'arrived_delivery',   label:'At Customer',   icon:'🏭', color:'#3b82f6', status:'at_customer',    toast:'Arrived at delivery logged' },
]

// ── ISSUE ACTIONS — panel with AI ─────────────────────────────────────────────
const ISSUE_ACTIONS = [
  ...
  { id:'defect',       label:'Vehicle Issue', icon:'🔧', color:'#8a9099' },
]
...
const STATUS_COLORS = {
  ...
  'at_risk':       { dot:'#ef4444', label:'AT RISK',       border:'rgba(239,68,68,0.25)',  bg:'rgba(239,68,68,0.04)' },
...
export default function DriverApp() {
  ...
  // ── ROUTINE LOG — instant, with undo for Delivered ───────────────────────
  function logRoutine(action) {
    ...
    setJobs(prev => {
      const updated = prev.map(j => j.ref === job.ref ? { ...j, status: action.status, alert: null } : j)
      if (isComplete) return [...updated.filter(j=>j.status!=='completed'), ...updated.filter(j=>j.status==='completed')]
      return updated
    })
    setActiveJob(prev => ({ ...prev, status: action.status, alert: null }))
    ...
    if (isComplete) {
      ...
      if (undoTimerRef.current) clearTimeout(undoTimerRef.current)
      if (countdownRef.current) clearInterval(countdownRef.current)
      setPendingUndo({ job, previousStatus })
      setUndoCountdown(5)
      countdownRef.current = setInterval(() => {
        setUndoCountdown(n => { if (n <= 1) { clearInterval(countdownRef.current); return 0 } return n - 1 })
      }, 1000)
      undoTimerRef.current = setTimeout(() => { setPendingUndo(null); setUndoCountdown(0) }, 5000)
    } else {
  ...
  function undoDelivered() {
    ...
    const { job, previousStatus } = pendingUndo
    setJobs(prev => prev.map(j => j.ref === job.ref ? { ...j, status: previousStatus } : j))
    setActiveJob(job)
    setPendingUndo(null)
    setUndoCountdown(0)
    showToast('Delivery undone — back to previous status')
  }
  ...
  function closePanel() {
    if (parsedResult && panelState === 'result') {
      const alert = { ...parsedResult, actionType: panelAction, time: new Date().toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}) }
      setLastAlert(alert)
    ...
    setPanelOpen(false); setPanelAction(null); setPanelState('idle')
    setInputText(''); setParsedResult(null); setShowDetail(false); setResolvedEta('')
  }

  function reopenLastAlert() {
    if (!lastAlert) return
    setParsedResult(lastAlert); setPanelAction(lastAlert.actionType||'breakdown')
    ...
    try {
      const res = await fetch('/api/driver/resolve', {
        ...
        body: JSON.stringify({
          ...
          vehicle_reg:driverInfo.vehicleReg, ref:job?.ref, resolution:reason,
          location_description:gpsDescription, route:job?.route,
          sla_window:job?.sla_window, original_issue:panelAction,
        })
      })
      const data = await res.json()
  ...
  function buildPrompt(actionId) {
    ...
    const prompts = {
      ...
      cant_complete: `DRIVER HOURS / CAPACITY ISSUE. Driver ${driverInfo.name} (${driverInfo.vehicleReg}) cannot complete all remaining runs today. Location: ${loc}. Current job: ${job?.route||'unknown'}. Reason: ${inputText||'hours/delay'}. Remaining jobs that cannot be completed: ${remainingList||'none listed'}. Assess: which job to prioritise, customer notifications for abandoned jobs, relief vehicle requirement, exact ops escalation.`,
    }
  ...
  async function sendAlert() {
    ...
    // Mark remaining jobs AT RISK if can't complete
    if (panelAction === 'cant_complete') {
      setJobs(prev => prev.map(j =>
        j.status !== 'completed' && j.ref !== job?.ref ? {...j, status:'at_risk'} : j
      ))
    }
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0c0e',color:'#e8eaed',fontFamily:'IBM Plex Sans,sans-serif',paddingBottom:88}}>
      ...
      {/* Toast */}
      {toast && (
        <div style={{position:'fixed',top:16,left:'50%',transform:'translateX(-50%)',zIndex:999,padding:'10px 20px',borderRadius:8,background:toast.type==='error'?'#ef4444':'#111418',border:'1px solid rgba(0,229,176,0.3)',color:toast.type==='error'?'#fff':'#00e5b0',fontSize:13,fontWeight:500,whiteSpace:'nowrap',boxShadow:'0 4px 20px rgba(0,0,0,0.6)'}}>
          ✓ {toast.msg}
        </div>
      )}
      ...
      {/* Delivered undo bar */}
      {pendingUndo && (
        <div style={{position:'fixed',top:0,left:0,right:0,zIndex:998,padding:'14px 16px',background:'#1a1f26',borderBottom:'1px solid rgba(0,229,176,0.2)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div>
            <div style={{fontSize:13,color:'#e8eaed',fontWeight:500}}>📦 {pendingUndo.job.ref} marked as delivered</div>
            <div style={{fontSize:11,color:'#4a5260',marginTop:2}}>Committing in {undoCountdown}s...</div>
          </div>
          <button onClick={undoDelivered}
            style={{padding:'8px 18px',background:'rgba(239,68,68,0.12)',border:'1px solid rgba(239,68,68,0.4)',borderRadius:7,color:'#ef4444',fontWeight:600,fontSize:13,cursor:'pointer'}}>
            UNDO
          </button>
        </div>
      )}
      ...
      {/* NAV */}
      <div style={{padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid rgba(255,255,255,0.06)',background:'rgba(10,12,14,0.98)',position:'sticky',top: pendingUndo?52:0,zIndex:50,transition:'top 0.2s'}}>
      ...
      <div style={{padding:'12px 0 0'}}>
        ...
        {sortedJobs.map(job => {
          ...
          return (
            <div key={job.ref} onClick={() => !isDone && setActiveJob(job)}
              style={{margin:'0 12px 10px',padding:'14px',borderRadius:10,cursor:isDone?'default':'pointer',border:`1px solid ${isActive&&!isDone?'#00e5b0':sc.border}`,background:sc.bg,opacity:isDone?0.45:1,transition:'all 0.2s'}}>
              ...
              <div style={{fontSize:13,color:isDone?'#4a5260':'#8a9099',marginBottom:isDone?0:4}}>{job.route}</div>
              {!isDone && (
                <>
                  <div style={{display:'flex',gap:10,fontSize:11,color:'#4a5260',flexWrap:'wrap'}}>
                  ...
                  {isAtRisk&&<div style={{marginTop:7,padding:'7px 10px',background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.25)',borderRadius:6,fontSize:11,color:'#ef4444',fontWeight:500}}>⚠ Reassignment required — driver hours critical</div>}
                </>
              )}

              {/* Routine actions — active non-completed job only */}
              {isActive && !isDone && !isAtRisk && (
                <div style={{marginTop:14,paddingTop:12,borderTop:'1px solid rgba(255,255,255,0.06)'}}>
                  <div style={{fontSize:9,color:'#4a5260',fontFamily:'monospace',letterSpacing:'0.06em',marginBottom:10}}>LOG PROGRESS</div>

                  {/* 3 routine buttons in a row */}
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:8}}>
                    {ROUTINE_ACTIONS.map(action => (
                      <button key={action.id} onClick={e=>{e.stopPropagation();logRoutine(action)}}
                        style={{padding:'11px 4px',borderRadius:8,fontSize:11,fontWeight:500,cursor:'pointer',background:`${action.color}10`,border:`1px solid ${action.color}30`,color:action.color,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                        <span style={{fontSize:16}}>{action.icon}</span>
                        <span style={{textAlign:'center',lineHeight:1.2}}>{action.label}</span>
                      </button>
                    ))}
                  </div>

                  {/* Delivered — full width, separate, prominent */}
                  <button onClick={e=>{e.stopPropagation();logRoutine({id:'delivered',label:'Delivered',icon:'📦',color:'#00e5b0',status:'completed',toast:'Delivery confirmed'})}}
                    style={{width:'100%',padding:'13px',borderRadius:8,fontSize:14,fontWeight:600,cursor:'pointer',background:'rgba(0,229,176,0.08)',border:'1px solid rgba(0,229,176,0.3)',color:'#00e5b0',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
                    📦 Mark as Delivered
                  </button>
                </div>
              )}
        ...
        {/* ISSUE GRID */}
        <div style={{padding:'8px 16px 6px',fontSize:9,color:'#4a5260',fontFamily:'monospace',letterSpacing:'0.08em',marginTop:4}}>REPORT AN ISSUE</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:7,padding:'0 12px 12px'}}>
          {ISSUE_ACTIONS.map(action => (
            <button key={action.id} onClick={()=>openPanel(action.id)}
              style={{padding:'11px 4px',borderRadius:8,background:'#111418',border:'1px solid rgba(255,255,255,0.06)',display:'flex',flexDirection:'column',alignItems:'center',gap:5,cursor:'pointer',outline:'none'}}>
              <span style={{fontSize:20}}>{action.icon}</span>
              <span style={{fontSize:9,color:'#8a9099',textAlign:'center',lineHeight:1.3}}>{action.label}</span>
            </button>
          ))}
        </div>
      ...
      <div style={{position:'fixed',bottom:0,left:0,right:0,padding:'10px 12px 24px',background:'rgba(10,12,14,0.97)',borderTop:'1px solid rgba(255,255,255,0.06)',backdropFilter:'blur(10px)',zIndex:100,display:'grid',gridTemplateColumns:'2fr 2fr 1.2fr',gap:8}}>
        <button onClick={()=>openPanel('breakdown')}
          style={{padding:'13px',background:'#ef4444',border:'none',borderRadius:10,color:'#fff',fontWeight:700,fontSize:13,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
          🚨 Breakdown
        </button>
        <button onClick={()=>openPanel('delayed')}
          style={{padding:'13px',background:'rgba(245,158,11,0.1)',border:'1px solid rgba(245,158,11,0.3)',borderRadius:10,color:'#f59e0b',fontWeight:600,fontSize:13,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
          ⏰ Running Late
        </button>
        <button onClick={()=>openPanel('cant_complete')}
          ...
          <span style={{lineHeight:1.2,textAlign:'center'}}>Can't Complete</span>
        </button>
      </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:200,display:'flex',flexDirection:'column',justifyContent:'flex-end'}}
          ...
          <div style={{background:'#111418',borderRadius:'16px 16px 0 0',maxHeight:'90vh',display:'flex',flexDirection:'column',borderTop:'1px solid rgba(255,255,255,0.08)'}}>
            ...
            <div style={{padding:'16px 20px 12px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
              <span style={{fontSize:12,fontWeight:600,fontFamily:'monospace',letterSpacing:'0.06em',color:panelAction==='breakdown'?'#ef4444':panelAction==='cant_complete'?'#ef4444':panelAction==='delayed'?'#f59e0b':'#00e5b0'}}>
                {panelAction==='breakdown'?'BREAKDOWN ALERT':panelAction==='cant_complete'?'CANNOT COMPLETE RUNS':panelAction==='rest'?'REST BREAK':panelAction==='temp_check'?'TEMPERATURE LOG':panelAction==='delayed'?'RUNNING LATE':panelAction==='defect'?'VEHICLE DEFECT':'FUEL STOP'}
              </span>
            ...
            <div style={{overflowY:'auto',flex:1,padding:'0 20px 32px'}}>
              ...
              <div style={{padding:'9px 11px',background:'#0d1014',borderRadius:7,display:'flex',alignItems:'center',gap:9,marginBottom:14}}>
                ...
                {gpsStatus!=='got'&&gpsStatus!=='getting'&&<button onClick={getGPS} style={{padding:'4px 10px',background:'rgba(0,229,176,0.08)',border:'1px solid rgba(0,229,176,0.2)',borderRadius:5,color:'#00e5b0',fontSize:11,cursor:'pointer'}}>Get GPS</button>}
              </div>
              ...
              {panelState==='result'&&parsedResult&&(()=>{
                ...
                return (
                  <div>
                    ...
                    <button onClick={closePanel} style={{width:'100%',padding:14,background:'#00e5b0',border:'none',borderRadius:8,color:'#000',fontWeight:600,fontSize:15,cursor:'pointer',marginBottom:8}}>Got it — close</button>
                    {panelAction!=='cant_complete'&&(
              ...
              {panelState==='resolving'&&(
                <div>
                  ...
                  <div style={{fontSize:12,color:'#4a5260',marginBottom:14}}>Ops notified. Job updates to on-track.</div>
                  {[
                    ...
                    <button key={opt.id} onClick={()=>resolveIssue(opt.label)}
                      ...
                      <div><div style={{fontSize:14,color:'#e8eaed',fontWeight:500}}>{opt.label}</div><div style={{fontSize:11,color:'#4a5260',marginTop:1}}>{opt.sub}</div></div>
              ...
              {panelState==='resolving_loading'&&<div style={{textAlign:'center',padding:'40px 0'}}><div style={{width:36,height:36,border:'3px solid rgba(0,229,176,0.15)',borderTop:'3px solid #00e5b0',borderRadius:'50%',margin:'0 auto 14px',animation:'spin 1s linear infinite'}}/><div style={{fontSize:13,color:'#00e5b0',fontFamily:'monospace'}}>NOTIFYING OPS...</div><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>}
              ...
              {panelState==='idle'&&(
                <>
                  ...
                  {panelAction==='cant_complete'&&(
                    <div style={{padding:'10px 12px',background:'rgba(239,68,68,0.06)',border:'1px solid rgba(239,68,68,0.2)',borderRadius:8,marginBottom:12}}>
                      <div style={{fontSize:12,color:'#ef4444',fontWeight:500,marginBottom:3}}>⛔ Remaining jobs will be flagged AT RISK</div>
                      <div style={{fontSize:11,color:'#8a9099'}}>Ops will be notified immediately. AI will prioritise which job to attempt and draft customer notifications for the rest.</div>
                    </div>
                  )}
                  ...
                  {['breakdown','delayed','temp_check','defect','cant_complete'].includes(panelAction)&&(
                    <textarea value={inputText} onChange={e=>setInputText(e.target.value)} rows={3}
                      placeholder={panelAction==='breakdown'?'What happened? e.g. Engine warning light, white smoke, M62 J25':panelAction==='delayed'?'What is causing the delay?':panelAction==='temp_check'?'e.g. 3.2°C probe reading':panelAction==='cant_complete'?'Reason — e.g. hours almost up, major delay on first run':'Describe the issue...'}
                  ...
                  <button onClick={sendAlert}
                    style={{width:'100%',padding:14,background:['breakdown','cant_complete'].includes(panelAction)?'#ef4444':'#00e5b0',border:'none',borderRadius:8,color:['breakdown','cant_complete'].includes(panelAction)?'#fff':'#000',fontWeight:600,fontSize:15,cursor:'pointer',marginBottom:8}}>
                    {panelAction==='breakdown'?'🚨 Alert ops now':panelAction==='cant_complete'?'⛔ Flag and alert ops':panelAction==='rest'?'🛌 Find safe parking →':panelAction==='temp_check'?'🌡 Log & check →':'⚠ Get instructions →'}
                  </button>
                  <div style={{fontSize:11,color:'#4a5260',textAlign:'center'}}>
                    {['breakdown','delayed','defect','temp_check','rest','cant_complete'].includes(panelAction)?'Ops manager notified immediately':'Instructions in seconds'}
                  </div>
                </>
              )}
            </div>
