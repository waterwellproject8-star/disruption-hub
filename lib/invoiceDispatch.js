...
import { deliverCallback, nextAttemptISO } from './callbackDelivery.js'

// ── HMAC callback fire with logging ─────────────────────────────────────────
// Inserts callback_log row, delivers synchronously, updates row with outcome.
// On failure, schedules retry via next_attempt_at for the retry cron.
async function fireInvoiceCallback(db, apiKeyId, eventType, payload) {
  ...
  const maxAttempts = 4
  const fullPayload = { event_type: eventType, ...payload }
  const eventRef = payload?.invoice_id || payload?.invoice_ref || payload?.ref || 'unknown'
  ...
  try {
    const { data: logRow, error: insertErr } = await db
      ...
      .insert({
        ...
        callback_secret: keyRow.callback_secret || null,
        payload: fullPayload,
        attempt_number: 1,
      ...
      .single()
    if (insertErr) {
      console.error('[invoice-callback] log insert failed:', insertErr.message)
    } else {
      logId = logRow?.id
    }
  } catch (err) {
    console.error('[invoice-callback] log insert threw:', err.message)
  }

  // 2. Synchronous first attempt
  ...
  console.log(`[invoiceDispatch] callback ${eventType} → ${keyRow.callback_url} status:${result.status_code || 'err'} ok:${result.ok}`)
  ...
  if (logId) {
    try {
      if (result.ok) {
        ...
        const nextISO = nextAttemptISO(1, maxAttempts)
        await db.from('callback_log').update({
          status_code: result.status_code,
          response_body: result.response_body || null,
          last_error: result.error || `http_${result.status_code}`,
          next_attempt_at: nextISO,
          ...(nextISO ? {} : { failed_at: new Date().toISOString() })
        }).eq('id', logId)
      }
    } catch (updateErr) {
      console.error('[invoice-callback] log update failed:', updateErr.message)
    }
  }
