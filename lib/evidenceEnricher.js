...
async function fetchWebhookEvents(db, clientId, jobRef, shipmentCreatedAt) {
  try {
    // Time window: events from 24h before shipment creation to 7d after.
    // If no shipment context, fall back to last 14 days.
    let fromTs, toTs
    if (shipmentCreatedAt) {
      const shipMs = new Date(shipmentCreatedAt).getTime()
      fromTs = new Date(shipMs - 24 * 60 * 60 * 1000).toISOString()
      toTs = new Date(shipMs + 7 * 24 * 60 * 60 * 1000).toISOString()
    } else {
      fromTs = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString()
...
export async function enrichEvidence(db, clientId, jobRef) {
  ...
  // Fetch shipment first to derive event time window, then everything else in parallel.
  const shipment = await fetchShipment(db, clientId, jobRef)
  const [progress, events, incidents] = await Promise.all([
    fetchLatestProgress(db, clientId, jobRef),
    fetchWebhookEvents(db, clientId, jobRef, shipment?.created_at),
