import { vocabFor } from './sectorVocabulary.js'
...
export function buildConsigneeVoiceMessage({ consigneeName, clientName, vehicleReg, revisedEta, delayMinutes, reason, ref, sector }) {
  const v = vocabFor(sector)
  const spokenReg = vehicleReg ? vehicleReg.replace(/\s/g,'').split('').join(', ') : null

  const parts = [
    `Hello. This is an automated ${v.delay_subject} update from ${clientName || 'your supplier'}.`
  ]

  if (consigneeName) {
    parts.push(`This message is for the ${v.voice_company_role} ${consigneeName}.`)
  }
  ...
  if (ref)        parts.push(`${v.delivery_ref_label}: ${spellOut(ref)}.`)
...
export function buildDriverInstructionSMS({ driverName, action, carrierName, carrierPhone, script, ref, sector }) {
  const lines = [
