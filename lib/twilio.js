...
<Response>
  <Pause length="1"/>
  <Say voice="Polly.Amy-Generative" language="en-GB">${safe}</Say>
  <Pause length="1"/>
  <Say voice="Polly.Amy-Generative" language="en-GB">I will repeat that message.</Say>
  <Pause length="1"/>
  <Say voice="Polly.Amy-Generative" language="en-GB">${safe}</Say>
  <Pause length="1"/>
  <Say voice="Polly.Amy-Generative" language="en-GB">End of message from DisruptionHub.</Say>
</Response>`
...
// Use when: delivery will be late due to breakdown, traffic, or incident
// Tone: professional, courteous — customer-facing call to a goods-in team
export function buildConsigneeVoiceMessage({ consigneeName, clientName, vehicleReg, revisedEta, delayMinutes, reason, opsPhone, ref }) {
  const spokenReg      = vehicleReg ? vehicleReg.replace(/\s/g,'').split('').join(', ') : null
  const spokenOpsPhone = opsPhone   ? opsPhone.replace(/\s/g,'').split('').join(', ')   : null

  const parts = [
    `Hello. This is an automated message from ${clientName || 'your supplier'}.`
  ]
  ...
  if (delayMinutes) parts.push(`The delay is approximately ${delayMinutes} minutes.`)
  if (revisedEta)   parts.push(`Revised estimated arrival is ${revisedEta}.`)
  if (reason)       parts.push(`Reason: ${reason.substring(0, 150)}.`)
  ...
  if (ref) parts.push(`Reference number: ${ref}.`)
