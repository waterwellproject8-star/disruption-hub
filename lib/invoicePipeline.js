...
// ── Deterministic delta computation from matched rate card ──────────────────
function findRateCardForLine(rateCards, line) {
  if (!Array.isArray(rateCards) || !line) return null
  const norm = (s) => String(s || '').toLowerCase().trim()
  return rateCards.find(rc =>
    norm(rc.lane_origin) === norm(line.lane_origin) &&
    norm(rc.lane_destination) === norm(line.lane_destination) &&
    (norm(rc.service_type) === norm(line.service_type) || !rc.service_type || !line.service_type)
  ) || null
}

function computeExpectedFromRateCard(line, rateCard) {
  if (!rateCard) return null
  const base = Number(rateCard.agreed_rate_total)
  const maxFuelPct = Number(rateCard.fuel_surcharge_pct_max)
  const appliedPct = Number(line?.fuel_surcharge_pct)

  if (!Number.isFinite(base) || base <= 0) return null
  if (!Number.isFinite(maxFuelPct)) return null

  const effectivePct = Number.isFinite(appliedPct)
    ? Math.min(appliedPct, maxFuelPct)
    : maxFuelPct

  const expected = base * (1 + effectivePct / 100)
  return Math.round(expected * 100) / 100
}

function recomputeDeltaDeterministically(discrepancy, lineItems, rateCards) {
  const line = (lineItems || []).find(li =>
    String(li.job_ref || '').trim() === String(discrepancy.job_ref || '').trim()
  )
  if (!line) {
    return { ...discrepancy, delta_source: 'ai_estimated' }
  }

  const rateCard = findRateCardForLine(rateCards, line)
  if (!rateCard) {
    return { ...discrepancy, delta_source: 'ai_estimated' }
  }

  const expected = computeExpectedFromRateCard(line, rateCard)
  if (expected === null) {
    return { ...discrepancy, delta_source: 'ai_estimated' }
  }

  const charged = Number(line.charged) || Number(discrepancy.charged) || 0
  const delta = Math.max(0, Math.round((charged - expected) * 100) / 100)

  const aiDelta = Number(discrepancy.delta) || 0
  if (Math.abs(aiDelta - delta) > 0.50) {
    console.warn(
      `[invoice-pipeline] delta override for ${discrepancy.job_ref}: ` +
      `AI returned ${aiDelta}, rate card computed ${delta}. Using rate card value.`
    )
  }
  ...
  return {
    ...
    expected,
    delta,
    delta_source: 'rate_card_computed'
  }
}
...
export async function auditInvoice({ db, clientId, fileContent, fileType, filename, apiKeyId = null }) {
  ...
  // Day 8.7: replace AI-supplied delta/expected with deterministic values
  // from matched rate card. AI keeps issue_type + evidence; pipeline owns the math.
  const correctedDiscrepancies = validatedDiscrepancies.map(d =>
    recomputeDeltaDeterministically(d, parsed.line_items, rateCards)
  )

  const genuineDiscrepancies = correctedDiscrepancies.filter(d => Number(d.delta) > 0)
  const unvalidatedDiscrepancies = correctedDiscrepancies.filter(d =>
