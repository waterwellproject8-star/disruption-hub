...
// ── Main pipeline ──────────────────────────────────────────────────────────
export async function auditInvoice({ db, clientId, fileContent, fileType, filename }) {
  ...
  const [rateCardsResult, shipmentRatesResult, clientContextResult] = await Promise.all([
    db.from('rate_cards')
  ...
  const rateCards = rateCardsResult.data || []
  const shipmentRates = {}
  ...
  // 5. Defensive filter — drop non-positive deltas
