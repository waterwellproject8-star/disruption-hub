...
- **Your job is qualitative classification, not arithmetic.** For each invoice line
  you flag, return job_ref, issue_type, and an evidence string explaining why the
  line appears wrong. Numerical fields (charged, expected, delta) MAY be included
  for traceability but the pipeline will recompute them deterministically from the
  matched rate card whenever one exists. Your numbers will be overridden in that
  case. Focus your effort on accurate issue_type selection and well-grounded
  evidence prose citing contract_ref.
- **MUST flag every detectable overcharge.** If a line's charged amount appears to
  exceed what the matched rate card permits (base rate * (1 + capped fuel surcharge
  percentage)), you MUST emit a discrepancy entry. Do not skip lines because the
  deviation is small or the issue type is borderline — the pipeline filters and
  recomputes; your role is recall.
- **One discrepancy per line.** Each invoice line produces AT MOST ONE entry.
  Compounding issues (wrong base AND surcharge breach) describe in the evidence
  string under a single dominant issue_type.
