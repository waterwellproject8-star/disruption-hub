...
- **You MUST flag every detectable overcharge.** If for any invoice line the charged
  amount exceeds the contractually-allowed maximum (computed from the matched rate
  card: base rate + capped fuel surcharge + contracted accessorials), you MUST emit
  a discrepancy entry with the correct delta. Skipping a clear overcharge because
  the margin is small (e.g. <5%) or because the issue type is ambiguous is WRONG.
  The "conservative deltas" guidance below applies to consolidating duplicate entries
  on the same line, NEVER to omitting a genuine overcharge. If you have a verified
  rate card and a charged total that exceeds the contracted total, emit the discrepancy.
- **One discrepancy per line.** Each invoice line produces AT MOST ONE entry in
  discrepancies[]. If a single line has multiple compounding issues (e.g., wrong
  base rate AND a fuel surcharge violation), select the issue_type that best
  characterises the DOMINANT cause and explain compounding factors in the evidence
  string. NEVER emit two discrepancies that point at the same (job_ref, charged,
  expected) tuple — those values mathematically yield exactly one delta. Doing so
  produces a self-contradictory dispute letter.
- **Math invariant.** For any single invoice line, sum of deltas across all
  discrepancies referencing that line MUST equal (line_charged - line_expected).
  If you cannot reconcile to that equality, prefer reporting fewer discrepancies
  with conservative deltas over inventing residual variances.
